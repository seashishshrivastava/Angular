 ...
 */
angular.module('App').controller('PatientCtrl', function ($scope, $rootScope, $element, patientService) {
	...
	$scope.SubmitPatientDetails = function(){ 
		setTimeout(function(){
			if($('.form-group').hasClass('has-error')){
				...
				$scope.patientInfo.LoginSessionKey = $scope.LoginSessionKey
				patientService.AddPatient($scope.patientInfo).then(function (response) {
				
		            if (response.ResponseCode == "200") {
		            	toastr.success(response.Message);
		            	if(response.Data.Type == 'add'){
		            		$scope.resetForm('AddPatient');
		            	}else{ 
		            		$('.form-group').removeClass('has-error');
							$('.form-group').removeClass('has-success');
							$('.help-block').remove();
	...
	$scope.getSearchPatientList = function(LoginSessionKey,init){
		if($scope.search !=''){
			...
			$scope.getPatientList(LoginSessionKey,init);
		}else if($scope.search == '' && $scope.IsSearchActive == 1){
			if($scope.search == '')
			{
				$scope.IsSearchActive = 0;
			}
			$scope.SearchLoading = true;
			$scope.getPatientList(LoginSessionKey,init);
		}
	...
	$scope.getPatientList = function(LoginSessionKey,init){
		$scope.LoginSessionKey = LoginSessionKey;
		...
		patientService.getPatientList(reqData).then(function (response) {
            if (response.ResponseCode == "200") {
	...
	$scope.update_appoint_status  = function(Patient,$index)
	{

		//$scope.PatientLoading = true;
	
		var reqData = {LoginSessionKey:$scope.LoginSessionKey,PatientID:Patient.PatientID,CalenderID:Patient.CalenderID,StatusID:Patient.StatusID}; 
		
		patientService.updateAppointment(reqData).then(function (response) {
	...
	$scope.AppointmentRecord = 0;
	$scope.PageNoApp = 1;
	$scope.PageRecordApp = 10;
	$scope.patientAppointment = function(LoginSessionKey,PatientID,Action){
		$scope.LoginSessionKey = LoginSessionKey;
		...
		var reqData = {LoginSessionKey:LoginSessionKey,PageNo:$scope.PageNoApp,PageRecord:$scope.PageRecordApp,Search:$scope.search,PatientID:PatientID}; 
		
		patientService.appointmentList(reqData).then(function (response) {
            	
            if (response.ResponseCode == "200") {
            	
            	$scope.TotalRecord = response.Data.TotalRecords;;
            	$scope.AppointmentRecord = response.Data.TotalRecords;
            	
            	if(response.Data.CalendraList.length>0)
            	{
            		for(k in response.Data.CalendraList)
            		{
            			$scope.AppointmentList.push(response.Data.CalendraList[k]);
            		}
