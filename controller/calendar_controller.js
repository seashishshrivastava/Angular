...
angular.module('App').controller('CalendarCtrl', function ($scope, $rootScope, $element, calendarService) {
	...
	$scope.addAppointment = {"LoginSessionKey":'',"PatientID":'',"PatientName":'',"PatientMobile":'',"PatientEmail":'',"FromDate":'',"ToDate":'',"StartTime":'',"EndTime":'',"NotifyPatientByEmail":'1',"NotifyPatientBySMS":'1',"NotifyPhysioByEmail":'0',"NotifyPhysioBySMS":'0',"Notes":''};
	$scope.PreviousMonth = '';
				...
				setTimeout(function(){ 
					$('#fullCalendar').fullCalendar({
			            ...
			            select: function(start, end, allDay) {
			                ...
			                var nstart=moment(start).format('YYYY-MM-DD');
			                var estart=moment(end).format('YYYY-MM-DD');
			            	//alert('New Date:' +nstart+ ' == '+ estart);
			                /*$.ajax({
							    url: 'http://www.physiodesk.com/prodemo/calendar/DayOffDates',
							    data:'',			                   
							    type: 'POST',                    
							    success: function(response){    	
							        ...
							        $.each(array,function(i){
									   	var ndd5 = array[i];
									   	//alert('MyDate ='+ndd5+'=='+nstart);
									   	if(ndd5==nstart){
									   		//alert('Day Off');
									   		toastr.clear();
											toastr.error('Day off!');
									   		statusdayof = 1;
									   		return false;
